...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-bold mb-1"><i class="bx bx-edit me-2 text-primary"></i>Edit Payroll #{{ $payroll->id }}</h4>
            <p class="text-muted mb-0">Update staff payroll, attendance adjustments, and advance deductions</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.payrolls.update', $payroll->id) }}" id="payrollEditForm">
        @csrf
        @method('PUT')
        <div class="row">
            ...
            <div class="col-lg-7 mb-4">
                <div class="card shadow-sm border-0 h-100">
                    <div class="card-header bg-light py-3 border-bottom d-flex justify-content-between align-items-center">
                        ...
                        <span class="badge bg-label-{{ $payroll->status == 'paid' ? 'success' : 'warning' }} fs-7">
                    ...
                    <div class="card-body p-4">
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                @php
                                    $selectedStaffId = old('staff_id', $payroll->staff_id);
                                    $selectedStaff = $payroll->staff ?? ($selectedStaffId ? $staffs->firstWhere('id', $selectedStaffId) : null);
                                    $selectedStaffText = $selectedStaff ? ($selectedStaff->full_name . ' (' . ($selectedStaff->position ?? 'Staff') . ')') : '';
                                @endphp
                                <div class="custom-ajax-wrapper" id="staffPickerWrapper">
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control @error('staff_id') is-invalid @enderror" id="staff_search_input" placeholder="Type name or phone to search employee..." value="{{ $selectedStaffText }}" autocomplete="off" required>
                                        <button type="button" class="btn btn-outline-secondary" id="clear_staff_btn" style="display: {{ $selectedStaffId ? 'inline-block' : 'none' }};"><i class="bx bx-x"></i></button>
                                    </div>
                                    <input type="hidden" name="staff_id" id="staff_id" value="{{ $selectedStaffId }}" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="year" id="year" class="form-select @error('year') is-invalid @enderror" required>
                                    ...
                                    <option value="{{ $y }}" {{ old('year', $payroll->year) == $y ? 'selected' : '' }}>
                                        {{ $y }}
                                    </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" step="0.01" min="0" name="basic_salary" id="basic_salary" class="form-control @error('basic_salary') is-invalid @enderror" value="{{ old('basic_salary', number_format($payroll->basic_salary, 2, '.', '')) }}" required>
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" step="0.01" min="0" name="deductions" id="deductions" class="form-control text-danger @error('deductions') is-invalid @enderror" value="{{ old('deductions', number_format($payroll->deductions ?? 0, 2, '.', '')) }}">
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" step="0.01" min="0" name="advance_amount" id="advance_amount" class="form-control text-warning @error('advance_amount') is-invalid @enderror" value="{{ old('advance_amount', number_format($payroll->advance_amount ?? 0, 2, '.', '')) }}">
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" step="0.01" min="0" id="net_amount" class="form-control fw-bold fs-5 text-success bg-light" value="{{ number_format($payroll->net_amount, 2, '.', '') }}" readonly>
                                </div>
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
                            ...
                            <button type="submit" class="btn btn-primary px-4"><i class="bx bx-check me-1"></i> Update Payroll</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-5 mb-4">
                ...
                <div class="card shadow-sm border-0 mb-4">
                    <div class="card-header bg-light py-3 d-flex justify-content-between align-items-center border-bottom">
                        ...
                        <span class="badge bg-primary" id="monthYearBadge">Selected Month</span>
                    </div>
                    <div class="card-body p-3">
                        ...
                        <div id="attendanceEmptyState" class="text-center py-4 text-muted small d-none">
                ...
                <div class="card shadow-sm border-0">
                    ...
                    <div class="card-body p-3">
                        ...
                        <div id="advanceEmptyState" class="text-center py-4 text-muted small d-none">
                        ...
                        <div id="noAdvancesMessage" class="alert alert-success d-none py-2 px-3 small mb-0 mt-2">
                            <i class="bx bx-check-circle me-1"></i> No salary advance for this month.
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    $(document).ready(function () {
        ...
        let calcTimer;
        function calculatePayroll(isStaffChange = false, isInitial = false) {
            ...
            calcTimer = setTimeout(function () {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        // Update Inputs if not initial load with user edits
                        if (isStaffChange) {
                            deductionsInput.val(parseFloat(data.deductions || 0).toFixed(2));
                            advanceInput.val(advanceTotal.toFixed(2));
                            updateNetAmount();
                        } else if (!isInitial) {
                            if (!deductionsInput.data('user-modified')) {
                                deductionsInput.val(parseFloat(data.deductions || 0).toFixed(2));
                            }
                            if (!advanceInput.data('user-modified')) {
                                advanceInput.val(advanceTotal.toFixed(2));
                            }
                            updateNetAmount();
                        }
        ...
        // Initial load calculation for current payroll
        calculatePayroll(false, true);
    });
</script>
@endsection
