...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="btn-group">
            <a href="{{ route('admin.documents.reports.expiry', ['timeframe' => 'all']) }}" class="btn btn-sm {{ $filter === 'all' ? 'btn-primary' : 'btn-outline-primary' }}">All</a>
            <a href="{{ route('admin.documents.reports.expiry', ['timeframe' => 'today']) }}" class="btn btn-sm {{ $filter === 'today' ? 'btn-danger' : 'btn-outline-danger' }}">Today</a>
            <a href="{{ route('admin.documents.reports.expiry', ['timeframe' => '7_days']) }}" class="btn btn-sm {{ $filter === '7_days' ? 'btn-warning' : 'btn-outline-warning' }}">Next 7 Days</a>
            <a href="{{ route('admin.documents.reports.expiry', ['timeframe' => '15_days']) }}" class="btn btn-sm {{ $filter === '15_days' ? 'btn-warning' : 'btn-outline-warning' }}">Next 15 Days</a>
            <a href="{{ route('admin.documents.reports.expiry', ['timeframe' => '30_days']) }}" class="btn btn-sm {{ $filter === '30_days' ? 'btn-info' : 'btn-outline-info' }}">Next 30 Days</a>
            <a href="{{ route('admin.documents.reports.expiry', ['timeframe' => 'expired']) }}" class="btn btn-sm {{ $filter === 'expired' ? 'btn-dark' : 'btn-outline-dark' }}">Expired</a>
        </div>
    </div>
