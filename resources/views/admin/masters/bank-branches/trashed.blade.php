...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
        <div>
            <h5 class="mb-0">Bank Branches - Recycle Bin</h5>
        ...
        <div>
            <a href="{{ route('admin.masters.bank-branches.index') }}" class="btn btn-secondary"><i class="bx bx-arrow-back me-1"></i> Back to List</a>
            <a href="{{ route('admin.masters.bank-branches.create') }}" class="btn btn-primary"><i class="bx bx-plus me-1"></i> Add Branch</a>
        </div>
    </div>
...
<script>
    function handleRestore(id, name) {
        Swal.fire({ title: 'Restore Branch?', text: "Are you sure you want to restore '" + name + "'?", icon: 'question', showCancelButton: true, confirmButtonColor: '#696cff', cancelButtonColor: '#3085d6', confirmButtonText: 'Yes, restore it!' }).then((result) => {
            if (result.isConfirmed) { const form = document.getElementById('restore-form'); form.action = "{{ url('admin/masters/bank-branches') }}/" + id + "/restore"; form.submit(); }
        })
    }
    ...
    function handleForceDelete(id, name) {
        Swal.fire({ title: 'Permanently Delete?', text: "This will permanently delete '" + name + "'!", icon: 'error', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6', confirmButtonText: 'Yes, permanently delete!' }).then((result) => {
            if (result.isConfirmed) { const form = document.getElementById('force-delete-form'); form.action = "{{ url('admin/masters/bank-branches') }}/" + id + "/force-delete"; form.submit(); }
        })
    }
</script>
@endsection
