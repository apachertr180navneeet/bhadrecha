...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-3 gap-3">
        <div>
            <h5 class="fw-bold mb-1">Cities</h5>
        ...
        <div>
            ...
            <a href="{{ route('admin.masters.city.download-template') }}" class="btn btn-outline-secondary"><i class="bx bx-download me-1"></i> Template</a>
            <a href="{{ route('admin.masters.city.trashed') }}" class="btn btn-outline-danger"><i class="bx bx-trash me-1"></i> Recycle Bin</a>
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('admin.masters.city.index') }}" class="row g-3">
                <div class="col-md-4">
                    <input type="text" name="search" class="form-control" placeholder="Search by city or state..." value="{{ request('search') }}">
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-2">
                    <a href="{{ route('admin.masters.city.index') }}" class="btn btn-outline-secondary w-100">Clear</a>
                </div>
                @endif
            </form>
        </div>
                    ...
                    <tr>
                        ...
                        <td class="text-center text-nowrap">
                            ...
                            <form action="{{ route('admin.masters.city.toggle-status', $city->id) }}" method="POST" class="d-inline">@csrf
                                <button type="submit" class="btn btn-sm btn-icon btn-outline-{{ $city->status == 'active' ? 'warning' : 'success' }}" title="{{ $city->status == 'active' ? 'Deactivate' : 'Activate' }}"><i class="bx bx-{{ $city->status == 'active' ? 'pause' : 'play' }}"></i></button>
                            </form>
...
<div class="modal fade" id="importModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{ route('admin.masters.city.import') }}" enctype="multipart/form-data">
                @csrf
                <div class="modal-header"><h5 class="modal-title">Import Cities</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
    ...
    function handleDelete(id, name) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                form.action = "{{ url('admin/masters/city') }}/" + id;
                form.submit();
            }
        })
    }
