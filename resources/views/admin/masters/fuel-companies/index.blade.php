...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-3 gap-3">
        <div>
            <h5 class="fw-bold mb-1">Fuel Companies</h5>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb breadcrumb-style1 mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item">Masters</li>
                    <li class="breadcrumb-item active">Fuel Companies</li>
                </ol>
            </nav>
        </div>
        <div>
            ...
            <a href="{{ route('admin.masters.fuel-companies.download-template') }}" class="btn btn-outline-secondary"><i class="bx bx-download me-1"></i> Template</a>
            <a href="{{ route('admin.masters.fuel-companies.trashed') }}" class="btn btn-outline-danger"><i class="bx bx-trash me-1"></i> Recycle Bin</a>
            <a href="{{ route('admin.masters.fuel-companies.create') }}" class="btn btn-primary"><i class="bx bx-plus me-1"></i> Add Fuel Company</a>
        </div>
    </div>
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form method="GET" class="row g-3">
                ...
                <div class="col-md-2">
                    <a href="{{ route('admin.masters.fuel-companies.index') }}" class="btn btn-outline-secondary w-100">Clear</a>
                </div>
                @endif
            </form>
        </div>
    ...
    <div class="card">
                    ...
                    <tr>
                        ...
                        <td class="text-center text-nowrap">
                            ...
                            <form action="{{ route('admin.masters.fuel-companies.toggle-status', $fuelCompany->id) }}" method="POST" class="d-inline">@csrf
                                <button type="submit" class="btn btn-sm btn-icon btn-outline-{{ $fuelCompany->status == 'active' ? 'warning' : 'success' }}" title="{{ $fuelCompany->status == 'active' ? 'Deactivate' : 'Activate' }}"><i class="bx bx-{{ $fuelCompany->status == 'active' ? 'pause' : 'play' }}"></i></button>
                            </form>
        ...
        @if($fuelCompanies->hasPages())
        <div class="card-footer bg-transparent border-top py-3">{{ $fuelCompanies->links() }}</div>
        @endif
    </div>
</div>
...
<div class="modal fade" id="importModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{ route('admin.masters.fuel-companies.import') }}" enctype="multipart/form-data">
                @csrf
                <div class="modal-header"><h5 class="modal-title">Import Fuel Companies</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
    ...
    function handleDelete(id, name) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                form.action = "{{ url('admin/masters/fuel-companies') }}/" + id;
                form.submit();
            }
        })
    }
