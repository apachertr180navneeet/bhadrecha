...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-3 gap-3">
        <div>
            <h5 class="fw-bold mb-1">Edit GST Master</h5>
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('admin.masters.gst.update', $gst->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="gst_rate" class="form-control @error('gst_rate') is-invalid @enderror" 
                               placeholder="e.g., 5% GST, 18% GST" value="{{ old('gst_rate', $gst->gst_rate) }}" required>
                        @error('gst_rate')
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" name="percentage" step="0.01" class="form-control @error('percentage') is-invalid @enderror" 
                               placeholder="e.g., 5, 12, 18" value="{{ old('percentage', $gst->percentage) }}" required>
                        @error('percentage')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="description" class="form-control @error('description') is-invalid @enderror" 
                                  rows="3" placeholder="Enter GST description...">{{ old('description', $gst->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mt-4 d-grid gap-2 d-md-flex justify-content-md-end">
                    ...
                    <button type="submit" class="btn btn-primary">Update GST Master</button>
                </div>
            </form>
        </div>
    </div>
