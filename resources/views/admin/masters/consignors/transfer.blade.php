...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
        <div>
            <h5 class="mb-0">Transfer Consignor</h5>
    ...
    <div class="card">
        <div class="card-body">
            <div class="alert alert-info">
                <strong>Transferring:</strong> {{ $consignor->name }} ({{ $consignor->phone }})<br>
                <strong>Current Company:</strong> {{ $consignor->company->name ?? 'N/A' }} |
                <strong>Current Branch:</strong> {{ $consignor->branch->name ?? 'N/A' }}
            </div>
            <form method="POST" action="{{ route('admin.masters.consignors.transfer.update', $consignor->id) }}">
                @csrf @method('PUT')
                ...
                <div class="row g-3 mb-3">
                    <div class="col-md-6">
                        ...
                        <select name="company_id" id="company_id" class="form-select" required>
                            ...
                            <option value="{{ $c->id }}" {{ $consignor->company_id == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
                ...
                <div class="row g-3 mb-3">
                    <div class="col-md-6">
                        ...
                        <select name="branch_id" id="branch_id" class="form-select">
                            ...
                            <option value="{{ $b->id }}" {{ $consignor->branch_id == $b->id ? 'selected' : '' }}>{{ $b->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="mt-4 d-grid gap-2 d-md-flex">
                    <button type="submit" class="btn btn-warning"><i class="bx bx-transfer-alt"></i> Transfer Consignor</button>
                    <a href="{{ route('admin.masters.consignors.index') }}" class="btn btn-secondary">Cancel</a>
                </div>
            </form>
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        var initialBranchId = "{{ $consignor->branch_id }}";
        if (initialCompanyId) {
