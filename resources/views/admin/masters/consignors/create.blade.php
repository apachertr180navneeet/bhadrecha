...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
        <div>
            <h5 class="mb-0">Add Consignor</h5>
    ...
    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route('admin.masters.consignors.store') }}">
                @csrf
                @if(auth()->user()->isSuperAdmin())
                <div class="row g-3 mb-3">
                    <div class="col-md-6"><label class="form-label">Company *</label><select name="company_id" id="company_id" class="form-select" required><option value="">Select Company</option>@foreach(\App\Models\Company::where('status','active')->get() as $c)<option value="{{ $c->id }}" {{ old('company_id') == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>@endforeach</select></div>
                ...
                @endif
                <div class="row g-3">
                    <div class="col-md-4"><label class="form-label">Vendor Code</label><input type="text" name="vendor_code" class="form-control" value="{{ old('vendor_code') }}"></div>
                    ...
                    <div class="col-md-4"><label class="form-label">GSTIN</label><input type="text" name="gstin" class="form-control" value="{{ old('gstin') }}"></div>
                    ...
                    <div class="col-md-6"><label class="form-label">Pincode</label><input type="text" name="pincode" class="form-control" value="{{ old('pincode') }}"></div>
                ...
                <div class="mt-4 d-grid gap-2 d-md-flex"><button type="submit" class="btn btn-primary">Save Consignor</button> <a href="{{ route('admin.masters.consignors.index') }}" class="btn btn-secondary">Cancel</a></div>
            </form>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        if (initialCompanyId) {
            loadBranches(initialCompanyId, "{{ old('branch_id') }}");
        }
