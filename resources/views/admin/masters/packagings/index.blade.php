...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-3 gap-3">
        <div>
            <h5 class="fw-bold mb-1">Packagings</h5>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb breadcrumb-style1 mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item active">Packagings</li>
                </ol>
            </nav>
        </div>
        <div>
            ...
            <a href="{{ route('admin.masters.packagings.download-template') }}" class="btn btn-outline-secondary"><i class="bx bx-download me-1"></i> Template</a>
            <a href="{{ route('admin.masters.packagings.trashed') }}" class="btn btn-outline-danger"><i class="bx bx-trash me-1"></i> Recycle Bin</a>
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('admin.masters.packagings.index') }}" class="row g-3">
                <div class="col-md-4">
                ...
                <div class="col-md-2">
                    <a href="{{ route('admin.masters.packagings.index') }}" class="btn btn-outline-secondary w-100">Clear</a>
                </div>
                @endif
            </form>
        </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center text-nowrap">
                            ...
                            <form action="{{ route('admin.masters.packagings.toggle-status', $packaging->id) }}" method="POST" class="d-inline">@csrf
                                <button type="submit" class="btn btn-sm btn-icon btn-outline-{{ $packaging->status == 'active' ? 'warning' : 'success' }}" title="{{ $packaging->status == 'active' ? 'Deactivate' : 'Activate' }}"><i class="bx bx-{{ $packaging->status == 'active' ? 'pause' : 'play' }}"></i></button>
                            </form>
                    ...
                    <tr>
                        <td colspan="5" class="text-center py-4">
                            <p class="text-muted mb-0">No packagings found</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
...
<div class="modal fade" id="importModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{ route('admin.masters.packagings.import') }}" enctype="multipart/form-data">
                @csrf
                <div class="modal-header"><h5 class="modal-title">Import Packagings</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
    ...
    function handleDelete(id, name) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                form.action = "{{ url('admin/masters/packagings') }}/" + id;
                form.submit();
            }
        })
    }
