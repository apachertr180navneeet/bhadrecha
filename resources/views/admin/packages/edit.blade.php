...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-bold mb-0">Edit Package</h4>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.packages.update', $package->id) }}" method="POST" id="editPackageForm">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <div class="custom-ajax-wrapper" id="clientPickerWrapper">
                            <div class="input-group">
                                ...
                                @php
                                    $selectedCustId = old('customer_id', $package->customer_id);
                                    $custObj = $package->customer ?? ($selectedCustId ? $customers->firstWhere('id', $selectedCustId) : null);
                                    $custText = $custObj ? ($custObj->name . ($custObj->mobile ? ' (' . $custObj->mobile . ')' : '')) : '';
                                @endphp
                                <input type="text" class="form-control @error('customer_id') is-invalid @enderror" id="client_search_input" placeholder="Type name or phone to search customer..." value="{{ $custText }}" autocomplete="off" required>
                                <button type="button" class="btn btn-outline-secondary" id="clear_client_btn" style="display: {{ $selectedCustId ? 'inline-block' : 'none' }};"><i class="bx bx-x"></i></button>
                            </div>
                            <input type="hidden" name="customer_id" id="customer_id" value="{{ $selectedCustId }}" required>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="duration_id" id="duration_id" class="form-select" required>
                            ...
                            @foreach($durations as $duration)
                                <option value="{{ $duration->id }}" data-days="{{ $duration->days }}" {{ old('duration_id', $package->duration_id) == $duration->id ? 'selected' : '' }}>
                                    {{ $duration->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" name="start_date" id="start_date" class="form-control" value="{{ old('start_date', $package->start_date ?? ($package->created_at ? $package->created_at->format('Y-m-d') : date('Y-m-d'))) }}" required>
                        @error('start_date') <small class="text-danger">{{ $message }}</small> @enderror
                    </div>

                    <div class="col-md-6 mb-3">
                        <label class="form-label">End Date <span class="text-danger">*</span></label>
                        <input type="date" name="end_date" id="end_date" class="form-control" value="{{ old('end_date', $package->end_date) }}" required>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-sm align-middle bg-white mb-0" id="packageServicesTable">
                                    ...
                                    <tbody id="packageServicesBody">
                                        @php
                                            $servicesKeyed = $services->keyBy('id');
                                            $existingItems = $package->services_with_qty;
                                        @endphp
                                        @foreach($existingItems as $item)
                                            @php
                                                $sObj = $servicesKeyed->get($item['service_id']);
                                                $itemPrice = isset($item['price']) && $item['price'] !== null ? (float)$item['price'] : ($sObj ? (float)$sObj->price : 0);
                                            @endphp
                                            @if($sObj)
                                            <tr data-service-id="{{ $sObj->id }}">
                                                ...
                                                <td>
                                                    <input type="number" name="service_prices[]" class="form-control form-control-sm service-price" value="{{ number_format($itemPrice, 2, '.', '') }}" step="0.01" min="0" required>
                                                </td>
                                                <td>
                                                    <input type="number" name="service_quantities[]" class="form-control form-control-sm service-qty" value="{{ $item['qty'] }}" min="1" required>
                                                </td>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" name="amount" id="amount" class="form-control" value="{{ old('amount', $package->amount) }}" step="0.01" min="0" required>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" id="remaining" class="form-control" value="{{ number_format($package->remaining, 2, '.', '') }}" readonly style="background-color: #f8f9fa; font-weight: bold;">
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        function updateServicesSummary(isInitial = false) {
            let totalQty = 0;
            let totalAmount = 0;
            ...
            // Auto update Package Amount field if user is editing rows (not initial load if amount is custom)
            if (!isInitial && $('#packageServicesBody tr').length > 0) {
        ...
        // Run initial summary calculation on load
        updateServicesSummary(true);
        ...
        // Auto calculate on load if duration is set but end_date is empty (e.g. for old packages)
