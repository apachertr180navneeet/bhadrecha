@php
    ...
    foreach($bulties as $bulty) {
        ...
        if ($bulty->bultyDetail) {
            $qty = floatval(($bulty->bultyDetail->billed_qty ?? 0) != 0 ? $bulty->bultyDetail->billed_qty : (($bulty->bultyDetail->qty_mt ?? 0) != 0 ? $bulty->bultyDetail->qty_mt : ($bulty->bultyDetail->challan_qty ?? 0)));
    ...
    $netTotal = $totalAmount - $totalShortageAmount - $totalDamageAmount;
    ...
    // In Nathdwara format, GST is calculated but usually paid by recipient under RCM.
    // The invoice preview shows GST amounts, but grand total = net total (GST not added).
    $gstAmount = $netTotal * ($gstRate / 100);
        ...
        <table style="width: 100%; border-collapse: collapse; text-align: center; font-size: 10px; font-weight: bold;">
            ...
            <tbody>
                <tr style="border-bottom: 2px solid #000;">
                    ...
                    <td style="border-right: 2px solid #000; padding: 8px;">{{ $existingInvoice?->description ?? 'WALL PUTTY TRANSPORATION' }}</td>
                    <td style="border-right: 2px solid #000; padding: 8px;">{{ $rowBillNo }}</td>
                    <td style="border-right: 2px solid #000; padding: 8px;">{{ $rowMnNo }}</td>
                    <td style="border-right: 2px solid #000; padding: 8px;">{{ count($bulties) }}</td>
                    <td style="border-right: 2px solid #000; padding: 8px;">{{ number_format($totalMt, 3) }}</td>
                    <td style="border-right: 2px solid #000; padding: 8px;">{{ number_format($totalAmount, 2) }}</td>
                    <td style="border-right: 2px solid #000; padding: 8px;">{{ number_format($totalShortageAmount, 2) }}</td>
                    <td style="border-right: 2px solid #000; padding: 8px;">{{ number_format($totalDamageAmount, 2) }}</td>
                    <td style="padding: 8px;">{{ number_format($freightTotal, 2) }}</td>
                </tr>
                
                @if($gstType === 'CGST_SGST')
                    <tr>
                        <td colspan="7" style="border-right: 2px solid #000;"></td>
                        <td colspan="2" style="border-right: 2px solid #000; border-bottom: 1px solid #000; padding: 4px; text-align: right;">C GST {{ number_format($halfGst, 1) }}%</td>
                        <td style="padding: 4px; border-bottom: 1px solid #000;">{{ number_format($cgstVal, 2) }}</td>
                    </tr>
                    <tr>
                        <td colspan="7" style="border-right: 2px solid #000;"></td>
                        <td colspan="2" style="border-right: 2px solid #000; border-bottom: 2px solid #000; padding: 4px; text-align: right;">S GST {{ number_format($halfGst, 1) }}%</td>
                        <td style="padding: 4px; border-bottom: 2px solid #000;">{{ number_format($sgstVal, 2) }}</td>
                    </tr>
                @else
                    <tr>
                        <td colspan="7" style="border-right: 2px solid #000;"></td>
                        <td colspan="2" style="border-right: 2px solid #000; border-bottom: 2px solid #000; padding: 4px; text-align: right;">I GST {{ number_format($gstRate, 1) }}%</td>
                        <td style="padding: 4px; border-bottom: 2px solid #000;">{{ number_format($igstVal, 2) }}</td>
                    </tr>
                @endif
                <tr>
                    <td colspan="7" style="border-right: 2px solid #000;"></td>
                    <td colspan="2" style="border-right: 2px solid #000; border-bottom: 2px solid #000; padding: 4px; text-align: center;">TOTAL GST</td>
                    <td style="padding: 4px; border-bottom: 2px solid #000;">{{ number_format($gstTotalVal, 2) }}</td>
                </tr>
                <tr>
                    <td colspan="7" style="border-right: 2px solid #000;"></td>
                    <td colspan="2" style="border-right: 2px solid #000; border-bottom: 2px solid #000; padding: 4px; text-align: center;">GRAND TOTAL</td>
                    <td style="padding: 4px; border-bottom: 2px solid #000;">{{ number_format($grandTotal, 2) }}</td>
                </tr>
            </tbody>
        </table>
